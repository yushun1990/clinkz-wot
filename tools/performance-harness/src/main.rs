...
const GENERATOR: &str = "clinkz-wot-fixture-generator-v1";
const MANIFESTS: &[&str] = &[
...
fn load_contract(
    ...
    validate_json_schema(
        ...
        "performance-result-v1",
    )?;
    let requirements = load_requirements(root)?;
    let fixtures = load_fixtures(root, allow_pending_digests)?;
...
fn load_fixtures(
    ...
    for table in tables {
        ...
        let fixture = Fixture {
            id: id.clone(),
            profile: string_field(table, "profile", &id)?,
            ...
            recipe: string_field(table, "recipe", &id)?,
            content_sha256: string_field(table, "content_sha256", &id)?,
        };
        parse_recipe(&fixture.recipe)?;
...
fn load_manifest(
        ...
        [("workload", true, true), ("contention", false, false)]
    {
        ...
        for table in tables {
            ...
            if gating && !has_absolute_gate(table) {
                return Err(format!(
                    "{id} has no absolute numeric or deterministic gate"
                ));
            }
            if require_report && string_array_field(table, "report", &id)?.is_empty() {
...
fn has_absolute_gate(table: &Table) -> bool {
    table.iter().any(|(key, item)| {
        let numeric_budget = (key.ends_with("_max") || key.ends_with("_min"))
            && (item.as_integer().is_some() || item.as_float().is_some());
        let invariant = key.starts_with("require_") && item.as_bool() == Some(true);
        numeric_budget || invariant
    })
}
...
fn generate_fixture(fixture: &Fixture) -> Result<Vec<u8>, String> {
    ...
    let mut output = b"clinkz-wot-fixture-v1\0".to_vec();
    ...
    if document_bytes > 0 {
        let document = generate_document(&fixture.id, document_bytes, forms, fixture.seed)?;
...
fn generate_document(
    ...
    let mut prefix = format!(
        "{{\"@context\":\"https://www.w3.org/2022/wot/td/v1.1\",\"id\":\"urn:clinkz:fixture:{id}\",\"title\":\"Fixture-{seed:016x}\",\"forms\":["
    );
    for index in 0..forms {
        ...
        prefix.push_str(&format!(
            "{{\"href\":\"coap://fixture.invalid/{id}/{index}\",\"op\":\"readproperty\"}}"
        ));
    }
...
fn validate_result(case: &Case, fixture: &Fixture, path: &Path) -> Result<(), String> {
    ...
    expect_json_u64(object.get("schema_version"), "schema_version", 1)?;
    ...
    if case.gating && status == "characterization" {
        return Err(format!(
            "gating case {:?} cannot be characterization",
            case.id
        ));
    }
    ...
    if metrics.values().any(|value| value.as_f64().is_none()) {
        return Err("adapter result metrics must be numbers".to_owned());
    }
...
#[cfg(test)]
mod tests {
    use super::{Fixture, generate_document, generate_fixture, parse_recipe};

    #[test]
    fn generated_document_is_valid_json_at_exact_size() {
        let document = generate_document("FX-TEST-001", 4096, 8, 46)
        ...
        assert_eq!(value["forms"].as_array().map(Vec::len), Some(8));
    }
