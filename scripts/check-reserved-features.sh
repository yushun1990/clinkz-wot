...
if ! cargo check --locked -p clinkz-wot-protocol-bindings-zenoh --no-default-features --features zenoh-pico >"$OUTPUT_FILE" 2>&1; then
    cat "$OUTPUT_FILE" >&2
    echo "expected zenoh-pico platform-hook backend to compile" >&2
    exit 1
fi

if ! cargo test --locked -p clinkz-wot-protocol-bindings-zenoh --no-default-features --features zenoh-pico >"$OUTPUT_FILE" 2>&1; then
    cat "$OUTPUT_FILE" >&2
...
if cargo check --locked -p clinkz-wot-protocol-bindings-zenoh --features zenoh,zenoh-pico >"$OUTPUT_FILE_CONFLICT" 2>&1; then
