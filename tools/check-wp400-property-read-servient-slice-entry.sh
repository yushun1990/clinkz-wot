...
candidate_base="fcce9e69036459506a163ac73ef5542f92e5eb7f"
    ...
    if [[ -f "$attestation" ]]; then
        ...
        if [[ ${#fields[@]} -eq 2 ]]; then
            [[ "${fields[1]}" == "$candidate_base" ]] \
                || fail "local candidate is not based on the registered default revision"
            candidate=${fields[0]}
            ...
            [[ "$(git -C "$root" rev-parse "$candidate^")" == "$candidate_base" ]] \
                || fail "PR merge candidate is not the unique child of the registered base"
...
verify_candidate_topology() {
    ...
    [[ "$parent" == "$candidate_base" ]] \
        || fail "candidate is not the single child of its registered base"
    ...
    [[ "${actual[*]}" == "${expected[*]}" ]] \
        || fail "candidate diff is not the exact registered 18-path topology"
}
...
inspect_contract() {
    ...
    for mutation in \
        ...
        "host_static_semantic_divergence_is_rejected"; do
