...
use crate::{
    ...
    td_defaults::{FormContext, default_operations_for_context},
    thing::Thing,
    validate::{Validate, ValidateError, ValidationLevel},
};

// The retained-source census below is valid only for serde_json 1.0.149's
// default representations. Cargo feature unification can otherwise replace
// the BTreeMap-backed Map with an opaque-capacity IndexMap.
const _: () = assert!(
    size_of::<serde_json::Map<String, Value>>() == size_of::<BTreeMap<String, Value>>(),
    "clinkz-wot-td retained-source guard: unsupported serde_json preserve_order/IndexMap Map representation",
);

// `arbitrary_precision` replaces the allocation-free scalar Number with a
// private String-backed representation whose retained capacity is opaque.
const _: () = assert!(
    !needs_drop::<serde_json::Number>(),
    "clinkz-wot-td retained-source guard: unsupported serde_json arbitrary_precision/String-backed Number representation",
);
...
impl ValidatedThingCursor {
    ...
    fn process(&mut self, task: Task) -> Result<(), LimitFailure> {
        match task {
            ...
            Task::Operations {
                ...
                let operation = operations.next().expect("nonempty task");
                let read_property = read_property || *operation == Operation::ReadProperty;
                if !operations.as_slice().is_empty() {
                    self.tasks.push(Task::Operations {
                        operations,
                        property,
                        read_property,
                    });
                } else if property && read_property {
                    self.add_readable_property_form()?;
                }
                Ok(())
            }
            ...
            Task::SecurityExpression(mut expression) => {
                // `advance` is reached only after Task::charge consumed this
                // reference's SecurityBranches unit.
                let reference = expression.advance();
                expression.branches = expression.branches.checked_add(1).ok_or(LimitFailure {
                ...
                if let Some(SecurityScheme::Combo(combo)) =
                    expression.definitions.get(reference.name)
                {
                    ...
                    // Retain only borrowed iterators. No child is copied,
                    // enqueued, or advanced until its own later charge.
                    expression.push_children(&combo.all_of, next_depth);
                    expression.push_children(&combo.one_of, next_depth);
                }

                if expression.has_work() {
                    self.tasks.push(Task::SecurityExpression(expression));
                }
                Ok(())
            }
            Task::UriTemplate {
    ...
    fn process_form(&mut self, form: &'static Form, owner: FormOwner) -> Result<(), LimitFailure> {
        ...
        if let Some(operations) = &form.op {
            ...
            if !operations.is_empty() {
                self.tasks.push(Task::Operations {
                    operations: static_slice_iter(operations),
                    property: matches!(owner, FormOwner::Property(_)),
                    read_property: false,
                });
            }
        } else if let FormOwner::Property(property) = owner
            && default_operations_for_context(FormContext::Property(property))
                .contains(&Operation::ReadProperty)
        {
            // The omitted-op default is a fixed TD-owned slice of at most two
            // operations, not an externally sized list.
            self.add_readable_property_form()?;
        }
    ...
    fn add_readable_property_form(&mut self) -> Result<(), LimitFailure> {
    ...
    fn push_security_expressions(
        ...
        if !roots.is_empty() {
            self.tasks
                .push(Task::SecurityExpression(SecurityExpressionCursor::new(
                    static_slice_iter(roots),
                    definitions,
                )));
        }
        Ok(())
...
struct SecurityExpressionFrame {
    references: slice::Iter<'static, String>,
    depth: u64,
}

struct SecurityExpressionCursor {
    frames: Vec<SecurityExpressionFrame>,
    branches: u64,
    definitions: &'static BTreeMap<String, SecurityScheme>,
}

impl SecurityExpressionCursor {
    fn new(
        roots: slice::Iter<'static, String>,
        definitions: &'static BTreeMap<String, SecurityScheme>,
    ) -> Self {
        let frames = alloc::vec![SecurityExpressionFrame {
            references: roots,
            depth: 1,
        }];
        Self {
    ...
    fn advance(&mut self) -> SecurityReference {
        let (reference, frame_finished) = {
            let frame = self.frames.last_mut().expect("nonempty expression task");
            let name = frame.references.next().expect("nonempty expression frame");
            (
                SecurityReference {
                    name: name.as_str(),
                    depth: frame.depth,
                },
                frame.references.as_slice().is_empty(),
            )
        };
        if frame_finished {
            self.frames.pop();
    ...
    fn push_children(&mut self, children: &'static [String], depth: u64) {
        if !children.is_empty() {
            self.frames.push(SecurityExpressionFrame {
                references: children.iter(),
                depth,
            });
        }
    }
...
enum Task {
    ...
    SecurityExpression(SecurityExpressionCursor),
    UriTemplate {
        bytes: &'static [u8],
        offset: usize,
        variables: u64,
...
impl Task {
    fn charge(&self) -> Option<Charge> {
        let (class, semantic, has_work) = match self {
            ...
            Self::SecurityExpression(expression) => {
                (WorkClass::SecurityBranches, false, expression.has_work())
            }
...
mod tests {
    ...
    use super::*;
    use crate::affordance::InteractionHelper;
    ...
    fn cursor_at_task(thing: Thing, predicate: impl Fn(&Task) -> bool) -> ValidatedThingCursor {
        let mut cursor = ValidatedThingCursor::new(thing, GatewayDefaultV1::limits());
        for _ in 0..4096 {
            if cursor.tasks.last().is_some_and(&predicate) {
                return cursor;
            }
            let mut budget = unit_budget();
            cursor = match cursor.step(&mut budget, false) {
                ValidatedThingStep::Pending(cursor) => cursor,
                _ => panic!("fixture reached a terminal state before the target task"),
            };
        }
        panic!("census did not reach the target task");
    }

    #[test]
    fn locked_serde_json_default_representations_match_the_census_model() {
        assert_eq!(
            size_of::<serde_json::Map<String, Value>>(),
            size_of::<BTreeMap<String, Value>>()
        );
        assert!(!needs_drop::<serde_json::Number>());
    }

    #[test]
    fn explicit_property_operations_advance_only_after_each_document_charge() {
        let operations = [
        ...
        let property = PropertyAffordance::builder(DataSchema::string())
            .form(Form::builder("/property").op(operations).build().unwrap())
            .build()
            .unwrap();
        let thing = Thing::builder("operation progress")
            .nosec()
            .property("property", property)
            .build()
            .unwrap();
        let cursor = cursor_at_task(thing, |task| matches!(task, Task::Operations { .. }));

        let operation_state = |cursor: &ValidatedThingCursor| match cursor.tasks.last() {
            Some(Task::Operations {
                operations,
                property,
                read_property,
            }) => (operations.len(), *property, *read_property),
            _ => panic!("expected an operation task"),
        };
        assert_eq!(operation_state(&cursor), (operations.len(), true, false));
        assert_eq!(cursor.census.readable_property_form_count, 0);

        let mut zero = WorkBudget::new();
        let mut cursor = match cursor.step(&mut zero, false) {
            ValidatedThingStep::Pending(cursor) => cursor,
            _ => panic!("zero DocumentNodes budget must not inspect an operation"),
        };
        assert_eq!(operation_state(&cursor), (operations.len(), true, false));
        assert_eq!(cursor.census.readable_property_form_count, 0);

        for remaining in (1..operations.len()).rev() {
            let mut one = WorkBudget::new().with_remaining(WorkClass::DocumentNodes, 1);
            cursor = match cursor.step(&mut one, false) {
                ValidatedThingStep::Pending(cursor) => cursor,
                _ => panic!("one charge must inspect exactly one explicit operation"),
            };
            assert_eq!(operation_state(&cursor).0, remaining);
            assert_eq!(cursor.census.readable_property_form_count, 0);
        }

        let mut one = WorkBudget::new().with_remaining(WorkClass::DocumentNodes, 1);
        let cursor = match cursor.step(&mut one, false) {
            ValidatedThingStep::Pending(cursor) => cursor,
            _ => panic!("work after the operation list must remain pending"),
        };
        assert!(
            !matches!(cursor.tasks.last(), Some(Task::Operations { .. })),
            "the final charged operation must finish the iterator"
        );
        assert_eq!(cursor.census.readable_property_form_count, 1);
    }

    #[test]
    fn security_roots_and_combo_children_advance_one_charged_reference_at_a_time() {
        let mut roots = Thing::builder("security root progress")
            .security_definition("leaf", SecurityScheme::nosec());
        for _ in 0..8 {
            roots = roots.security_name("leaf");
        }
        let roots = roots.build().unwrap();
        let roots = cursor_at_task(roots, |task| matches!(task, Task::SecurityExpression(_)));

        let expression_state = |cursor: &ValidatedThingCursor| match cursor.tasks.last() {
            Some(Task::SecurityExpression(expression)) => (
                expression.branches,
                expression.frames.len(),
                expression
                    .frames
                    .last()
                    .expect("expression has a frame")
                    .references
                    .len(),
            ),
            _ => panic!("expected a security-expression task"),
        };
        assert_eq!(expression_state(&roots), (0, 1, 8));

        let mut no_security = WorkBudget::new()
        ...
        let roots = match roots.step(&mut no_security, false) {
            ValidatedThingStep::Pending(cursor) => cursor,
            _ => panic!("zero SecurityBranches budget must not advance any root"),
        };
        assert_eq!(expression_state(&roots), (0, 1, 8));

        let mut root_charge = WorkBudget::new().with_remaining(WorkClass::SecurityBranches, 1);
        let roots = match roots.step(&mut root_charge, false) {
            ValidatedThingStep::Pending(cursor) => cursor,
            _ => panic!("one charge must advance exactly one root"),
        };
        assert_eq!(expression_state(&roots), (1, 1, 7));

        let children = [
            "leaf", "leaf", "leaf", "leaf", "leaf", "leaf", "leaf", "leaf",
        ];
        let thing = Thing::builder("security progress")
            .security_definition("leaf", SecurityScheme::nosec())
            .security_named("outer", SecurityScheme::combo_one_of(children))
            .build()
            .unwrap();
        let cursor = cursor_at_task(thing, |task| matches!(task, Task::SecurityExpression(_)));
        assert_eq!(expression_state(&cursor), (0, 1, 1));

        let mut no_security = WorkBudget::new()
            .with_remaining(WorkClass::DocumentNodes, u64::MAX)
            .with_remaining(WorkClass::JsonSchemaNodes, u64::MAX)
            .with_remaining(WorkClass::UriBytes, u64::MAX);
        let cursor = match cursor.step(&mut no_security, false) {
            ValidatedThingStep::Pending(cursor) => cursor,
            _ => panic!("zero SecurityBranches budget must not advance the root"),
        };
        assert_eq!(expression_state(&cursor), (0, 1, 1));

        let mut combo_charge = WorkBudget::new().with_remaining(WorkClass::SecurityBranches, 1);
        let cursor = match cursor.step(&mut combo_charge, false) {
            ValidatedThingStep::Pending(cursor) => cursor,
            _ => panic!("one charge must advance the combo root only"),
        };
        assert_eq!(expression_state(&cursor), (1, 1, children.len()));

        let mut child_charge = WorkBudget::new().with_remaining(WorkClass::SecurityBranches, 1);
        let cursor = match cursor.step(&mut child_charge, false) {
            ValidatedThingStep::Pending(cursor) => cursor,
            _ => panic!("one further charge must advance one combo child only"),
        };
        assert_eq!(expression_state(&cursor), (2, 1, children.len() - 1));
    }
