...
attestation_rel="docs/audits/WP-200-property-read-route-reservation-review.toml"
attestation="$root/$attestation_rel"
fixture="$root/tools/compile-contracts/wp200-property-read-route-reservation"
schema="$root/tools/design-check/tests/wp200_property_read_route_reservation_schema.rs"
candidate_base="5a53a82a5d68a336b56b19e2e8f4c27f87492731"

fail() {
    echo "WP-200 Property Read route-reservation entry check: $*" >&2
    exit 1
}
    ...
    if [[ -f "$attestation" ]]; then
        ...
        if [[ ${#fields[@]} -eq 2 ]]; then
            ...
            git -C "$root" merge-base --is-ancestor \
                "${fields[1]}" "$candidate_base" \
                || fail "PR merge checkout first parent is not an ancestor of the registered base"
            candidate=${fields[2]}
            [[ "$(git -C "$root" rev-parse "$candidate^")" == "$candidate_base" ]] \
                || fail "PR merge candidate is not based on the registered base"
...
verify_candidate_topology() {
    local candidate parent parent_count
    ...
    [[ "${actual[*]}" == "${expected[*]}" ]] \
        || fail "candidate diff is not the exact registered 23-path topology"
}
...
inspect_contract() {
    for path in \
        ...
        "$fixture/Cargo.toml" \
        "$fixture/src/lib.rs" \
        "$fixture/tests/route_reservation.rs" \
        "$schema"; do
    ...
    for marker in \
        ...
        grep -Fq "$marker" "$fixture/src/lib.rs" \
            || fail "real route-reservation fixture is missing: $marker"
    done
    for forbidden in \
        ...
        "struct PrepareInput" \
        "clinkz_wot_servient"; do
        if grep -FRq "$forbidden" "$fixture/Cargo.toml" "$fixture/src" "$fixture/tests"; then
            fail "contract contains fixture-owned authority: $forbidden"
        fi
    done
}

run_prechecks() {
    bash -n "$root/tools/check-wp200-property-read-route-reservation-entry.sh"
    bash -n "$root/tools/check-wp200-property-read-route-reservation.sh"
...
require_preimplementation_failure() {
    local output status
    set +e
    output=$("$root/tools/check-wp200-property-read-route-reservation.sh" 2>&1)
    status=$?
    set -e
    if [[ $status -eq 0 ]]; then
    ...
    if [[ $status -ne 1 ]] \
        || ! grep -Fq 'Producer-route artifact reservation constructor is missing' <<<"$output"; then
        printf '%s\n' "$output" >&2
...
case "$mode" in
    --candidate)
        ...
        run_prechecks
        require_preimplementation_failure
        echo "WP-200 Property Read route-reservation entry check: candidate ready for independent review"
        ;;
    --reviewed)
        ...
        run_prechecks
        require_preimplementation_failure
        echo "WP-200 Property Read route-reservation entry check: reviewed candidate remains pre-source"
        ;;
    --admission-ready)
        [[ -f "$attestation" ]] || fail "independent review attestation is missing"
        ...
        run_prechecks
        require_preimplementation_failure
        echo "WP-200 Property Read route-reservation entry check: implementation admission ready"
        ;;
    *)
        echo "usage: tools/check-wp200-property-read-route-reservation-entry.sh [--candidate|--reviewed|--admission-ready]" >&2
        exit 2
        ;;
esac
