...
if rg -n --glob '*.rs' '\bSecurityError\b' "${source_roots[@]}" \
    ...
    echo "legacy API absence check: SecurityError remains" >&2
    exit 1
fi
...
for variant in \
    ...
    if rg -n --glob '*.rs' "CoreError::$variant\\b" "${source_roots[@]}" \
        ...
        echo "legacy API absence check: CoreError::$variant remains" >&2
        exit 1
    fi
done
...
if rg -n --glob '*.rs' "$legacy_names" "${source_roots[@]}"; then
    echo "legacy API absence check: legacy error name remains in source" >&2
    exit 1
fi
if rg -n "${public_doc_globs[@]}" "$legacy_names" "${public_docs[@]}"; then
    echo "legacy API absence check: legacy error name remains in public documentation" >&2
    exit 1
fi
...
for predicate in is_missing_handler is_security is_timeout; do
    if rg -n --glob '*.rs' "\\b$predicate\\b" "servient" "clinkz-wot" \
        ...
        echo "legacy API absence check: legacy Servient predicate $predicate remains" >&2
        exit 1
    fi
done

echo "legacy API absence check: removed public error APIs remain absent"
