...
mode=${1:---candidate}
attestation_rel="docs/audits/WP-100-logical-time-correction-review.toml"
attestation="$root/$attestation_rel"

fail() {
    echo "WP-100 logical time correction entry check: $*" >&2
    exit 1
}
...
run_state_check() {
    ...
    cargo run --locked --quiet \
        ...
        check-logical-time-correction-entry-state "$state"
}

run_prechecks() {
    bash -n "$root/tools/check-wp100-logical-time-correction-entry.sh"
    bash -n "$root/tools/check-wp100-logical-time-correction.sh"
...
require_preimplementation_failure() {
    local output status
    set +e
    output=$("$root/tools/check-wp100-logical-time-correction.sh" 2>&1)
    status=$?
    set -e
    if [[ $status -eq 0 ]]; then
    ...
    if [[ $status -ne 1 ]] \
        || ! grep -Fq \
            ...
            <<<"$output"; then
        printf '%s\n' "$output" >&2
        fail "completion check did not stop at the expected absent-method boundary"
    fi
}
...
case "$mode" in
    --candidate)
        ...
        run_prechecks
        require_preimplementation_failure
        echo "WP-100 logical time correction entry check: candidate ready for independent review"
        ;;
    --admission-ready)
        ...
        require_preimplementation_failure
        echo "WP-100 logical time correction entry check: implementation admission ready"
        ;;
    *)
        echo "usage: tools/check-wp100-logical-time-correction-entry.sh [--candidate|--admission-ready]" >&2
        exit 2
        ;;
esac
